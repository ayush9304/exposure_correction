...
def rgb2luminance(rgb, coeffs=[0.25, 0.5, 0.25]):
    if abs(1 - sum(coeffs)) > 1e-6:
        raise(f'coeffs sum to {sum(coeffs)}, which is not 1.')
    
    # luma = coeffs(1) * rgb(:,:,1) + coeffs(2) * rgb(:,:,2) + coeffs(3) * rgb(:,:,3)
    luma = np.dot(rgb[...,:3], coeffs)
    return luma
...
def getOrganisedInputImg(input_img, num_of_output_channels):
    ...
    for k in range(3):
        ...
        plane_flat = np.tile(plane_flat,num_of_output_channels)  #repeat num_of_output_channels times
